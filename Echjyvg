...
async def fetch_educator_by_uid(uid):
    ...
    # Try to fetch from API using uid
    url = f"https://unacademy.com/api/sheldon/v1/list/course?username=temp&limit=1"
    # Note: We can't directly fetch by UID from API, so we'll save what we have
    return None
...
async def send_scheduler_progress(username, thread_id, total_courses, total_batches, checked_courses, checked_batches, new_courses, new_batches, new_educators, current_phase):
    ...
    if progress_key not in scheduler_progress_messages or scheduler_progress_messages[progress_key] is None:
        try:
            msg = await bot.send_message(
                chat_id=thread_id.split('_')[0],
                message_thread_id=int(thread_id.split('_')[1]),
                text=progress_text
            )
    ...
    while scheduler_running:
        try:
            ...
            for doc in educators_col.find():
                ...
                # Upload new courses and batches
                for course in new_courses_list:
                ...
                if total_courses > 0:
                    ...
                    await send_scheduler_progress(username, f"{group_id}_{thread_id}", total_courses, total_batches, checked_courses, checked_batches, new_courses_count, new_batches_count, new_educators_counter['count'], "courses")
                        ...
                        try:
                            ...
                            if end_time_str != "N/A":
                                ...
                                if current_time_utc > end_time:
                                    # Mark as completed
                                    try:
                                        # Get channel_msg_id for updating group caption
                                    ...
                                    try:
                                        ...
                                        educators_col.update_one(
                                            ...
                                            {"$set": {
                                                ...
                                                "courses.$.channel_msg_id": new_channel_msg_id,
                ...
                if total_batches > 0:
                    ...
                    await send_scheduler_progress(username, f"{group_id}_{thread_id}", total_courses, total_batches, checked_courses, checked_batches, new_courses_count, new_batches_count, new_educators_counter['count'], "batches")
                                    ...
                                    try:
                                        ...
                                        educators_col.update_one(
                                            ...
                                            {"$set": {
                                                ...
                                                "batches.$.channel_msg_id": new_channel_msg_id,
                ...
                await send_scheduler_progress(username, f"{group_id}_{thread_id}", total_courses, total_batches, checked_courses, checked_batches, new_courses_count, new_batches_count, new_educators_counter['count'], "complete")
...
async def check_single_educator(username):
    ...
    doc = educators_col.find_one({"username": username})
    if not doc:
        print(f"Educator {username} not found")
        return
    ...
    for course in new_courses_list:
        ...
        course_data["teachers"] = teachers
        educators_col.update_one({"username": username}, {"$push": {"courses": course_data}})
        print(f"✓ Added new course {course['uid']}")
    ...
    for batch in new_batches_list:
        ...
        batch_data["teachers"] = teachers
        educators_col.update_one({"username": username}, {"$push": {"batches": batch_data}})
        print(f"✓ Added new batch {batch['uid']}")
    ...
    # Refresh and check existing items
    doc = educators_col.find_one({"username": username})
    courses_to_check = [c for c in doc.get("courses", []) if not c.get("is_completed", False) and c.get("msg_id")]
    batches_to_check = [b for b in doc.get("batches", []) if not b.get("is_completed", False) and b.get("msg_id")]
    ...
    # Check courses (same logic as in schedule_checker)
    # ... (rest of the checking logic follows the same pattern as schedule_checker)
