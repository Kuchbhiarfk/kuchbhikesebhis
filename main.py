...
def fetch_videos(url, chapter_hash):
    try:
        ...
        return [
            f"{i.find('div', class_='item-name').text.strip()}💀{chapter_hash}💀 : "
...
# Extract notes
def fetch_notes(url, chapter_hash):
    try:
        r = fetch_url(url)
        soup = BeautifulSoup(r.text, 'html.parser')
        items = soup.find_all('div', class_='item-box')[::-1]
...
# Extract DPP
def fetch_dpp(url, chapter_hash):
    try:
        r = fetch_url(url)
        soup = BeautifulSoup(r.text, 'html.parser')
        items = soup.find_all('div', class_='item-box')[::-1]
    ...
    try:
        ...
        if 'application/json' in r.headers.get('content-type', ''):
            ...
            for a in soup.find_all('a'):
                if a.get('href') and a.text.strip() and a.text.strip() != "← Back to Subjects":
...
def fetch_chapter_content(chap_link):
    try:
        ...
        lines = []
        lines.extend(fetch_videos(videos_url, chapter_hash))
        lines.extend(fetch_notes(notes_url, chapter_hash))
...
async def select_subjects(update: Update, context):
    try:
        ...
        subject_chapters = {}  # To store chapters for each subject

        # Count chapters and store chapter data
        for idx in selected_indices:
            ...
            chapters = [(a.text.strip(), base_url + a['href'] if a['href'].startswith('/') else a['href'])
                        for a in soup.find_all('a') if a.get('href') and a.text.strip() != "← Back to Subjects"]
            subject_chapters[sub_name] = chapters
        ...
        for s_idx, idx in enumerate(selected_indices, start=1):
            sub_name, sub_path = subject_list[idx]
            chap_url = base_url + sub_path
            chapters = subject_chapters[sub_name]
        ...
        # Send subject chapter list files
        for subject_filename in subject_files:
            with open(subject_filename, "rb") as f:
                await update.message.reply_document(document=f, filename=subject_filename)
            os.remove(subject_filename)

        # Send combined content file
        all_data = "\n".join(all_data_parts).strip()
        if all_data:
            filename = f"all_content_{uuid.uuid4()}.txt"
        ...
        await update.message.reply_text("✅ Done. All chapters fetched and subject files sent.")
        return SUBJECT_URL
