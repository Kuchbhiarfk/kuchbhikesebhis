...
async def url_handler(update: Update, context):
    ...
    try:
        ...
        if "use_token" in token_data:
            ...
            await update.message.reply_text("❌ Could not fetch session token.")
            return SUBJECT_URL
    except Exception as e:
        await update.message.reply_text(f"❌ Error fetching session token: {e}")
        return SUBJECT_URL

    # Step 2: Process subject URL
    url = update.message.text.split(maxsplit=1)[1] if len(update.message.text.split()) > 1 else ""
    if not url:
    ...
    try:
        ...
        await update.message.reply_text(msg + "\nSend subject indices (e.g. 1,3,5 or 0 for all)")
...
async def fetch_chapter_content(sub_name, chap_name, chap_link):
    async with sem:
        return await asyncio.to_thread(_fetch_chapter_content_sync, chap_link)

def _fetch_chapter_content_sync(chap_link):
    try:
...
async def select_subjects(update: Update, context):
    try:
        ...
        progress_message = await update.message.reply_text("Starting fetch...")
        ...
        for s_idx, idx in enumerate(selected_indices, start=1):
            ...
            chapter_tasks = [fetch_chapter_content(sub_name, chap_name, chap_link) for chap_name, chap_link in chapters]

            chapter_contents = []
            for c_idx, task in enumerate(asyncio.as_completed(chapter_tasks), start=1):
                content = await task
            ...
            ordered_clean = [c for c in chapter_contents if c.strip()]
            all_data_parts.extend(ordered_clean)
        ...
        await update.message.reply_text("✅ All subjects processed.")
        return SUBJECT_URL
...
def main():
    bot_token = "7639794663:AAHJZsQndmn3EOfU29hfC6ks-9-SVawrZFs"  # Replace with your token
