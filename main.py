...
# MongoDB connections
# Original MongoDB for educators data
...
# Global variable for optry progress
optry_progress_message = None

# Global variable to track last schedule check time
...
async def schedule_checker():
    ...
    global scheduler_progress_messages, last_schedule_check
    
    # Wait for 12 hours before first check after bot restart
    if last_schedule_check is None:
        print(f"\n{'='*60}")
...
async def send_optry_progress(total_educators, processed_educators, new_educators_found):
    ...
    progress_text = (
        ...
        f"Data Fetched & Uploaded: {processed_educators}/{total_educators}\n"
    ...
    try:
        ...
        for idx, optry_educator in enumerate(optry_educators, 1):
            try:
                ...
                # Check if these educators exist in optry MongoDB, if not add them
                for edu_username, edu_uid, edu_first, edu_last, edu_avatar in all_found_educators:
                    ...
                    # Check if exists in optry MongoDB
                    exists = collection_optry.find_one({"uid": edu_uid})
                    if not exists:
                        # Add to optry MongoDB
                        try:
